...
bool is_Title;//标志位，若此值为true说明CutStrCode()函数在代码行中找到了‘:’
...
int main(int argc, char* argv[])
{
	...
	if (argc == 1)return 0;
	for (i = 1; i < argc; ++i) {
		FileSize = 0;//储存生成的文件大小
	Number = 0;//储存偏移地址
	Line_num = 1;//行号
	is_Title = false;//标志位，若此值为true说明CutStrCode()函数在代码行中找到了‘:’,若表达式为真，判断后应重新赋值为false
		fin_src = argv[i];
		cout<<"code file="<<fin_src<<endl;
		if (fin_src.rfind(".asm") == 0) {//.asm为汇编代码文件，如果不是，正常编译，但提示后缀名出错
			cout << "The suffix name is not \".asm\"" << endl;//提示后缀名有问题，继续编译
		}
		n = fin_src.rfind(".");
		if (n == 0)return 0;
